...
class Test03GenreAPI:
    ...
    def test_01_genre_not_auth(self, client):
        ...
        assert response.status_code != 404, (
            "Страница `/api/v1/genres/` не найдена, "
            "проверьте этот адрес в *urls.py*"
        )
        assert response.status_code == 200, (
            "Проверьте, что при GET запросе `/api/v1/genres/` "
    ...
    def test_02_genre(self, admin_client):
        ...
        assert response.status_code == 400, (
            "Проверьте, что при POST запросе `/api/v1/genres/` "
            "нельзя создать 2 жанра с одинаковым `slug`"
        )
        ...
        assert len(data["results"]) == 1, (
            "Проверьте, что при GET запросе `/api/v1/genres/` "
            "фильтуется по search параметру названия жанра "
        )
    ...
    def test_03_genres_delete(self, admin_client):
        ...
        assert response.status_code == 405, (
            "Проверьте, что при GET запросе `/api/v1/genres/{slug}/` "
            "возвращаете статус 405"
        )
        response = admin_client.patch(f'/api/v1/genres/{genres[0]["slug"]}/')
        assert response.status_code == 405, (
            "Проверьте, что при PATCH запросе `/api/v1/genres/{slug}/` "
            "возвращаете статус 405"
        )
    ...
    def test_05_genre_create_user(self, user_client):
        ...
        assert response.status_code == code, (
            ...
            f"создание жанров недоступно для "
