...
class Test05ReviewAPI:
    ...
    def test_01_review_not_auth(self, client, admin_client):
        ...
        assert response.status_code != 404, (
            "Страница `/api/v1/titles/{title_id}/reviews/` не найдена,"
            " проверьте этот адрес в *urls.py*"
        )
        assert response.status_code == 200, (
            "Проверьте, что "
            "при GET запросе `/api/v1/titles/{title_id}/reviews/` "
    ...
    def create_review(self, client_user, title_id, text, score):
        ...
        assert response.status_code == 201, (
            "Проверьте, что "
            "при POST запросе `/api/v1/titles/{title_id}/reviews/` "
    ...
    def test_02_review_admin(self, admin_client, admin):
        ...
        assert response.status_code == code, (
            ...
            "на `/api/v1/titles/{title_id}/reviews/` "
            "нельзя добавить второй отзыв "
            "на то же самое произведение, и возвращается "
            f"статус {code}"
        )
        try:
        ...
        assert response.status_code == code, (
            "Проверьте, что "
            "PUT запрос на `/api/v1/titles/{title_id}/reviews/`"
        ...
        assert review.get("score") == 5, (
            ...
            "возвращаете данные с пагинацией. "
            "Значение параметра `results` неправильное, "
            "`score` не найдено или не сохранилось при POST запросе"
        )
    ...
    def test_03_review_detail(self, client, admin_client, admin):
        ...
        assert response.status_code != 404, (
            "Страница `/api/v1/titles/{title_id}/reviews/{review_id}/` "
        ...
        assert data.get("score") == 10, (
            "Проверьте, что при PATCH "
            "запросе `/api/v1/titles/{title_id}/reviews/{review_id}/` "
        ...
        assert response.status_code == 403, (
            ...
            "при попытки изменить не свой отзыв возвращается статус 403"
        )
        ...
        assert len(test_data) == len(reviews) - 1, (
            "Проверьте, что при DELETE запросе "
            "`/api/v1/titles/{title_id}/reviews/{review_id}/` удаляете объект"
        )
